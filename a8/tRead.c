...
void interrupt_service_routine () {
  uthread_t u_t = queue_dequeue(&prq);
  uthread_unblock(u_t);
  // TODO FINISHED YO
}
...
void run (int num_blocks) {
  ...
  for (int thread_num = 0; thread_num<num_blocks; thread_num++){
    intptr_t thread_num_ptr = (intptr_t) thread_num;
    thread [thread_num] = uthread_create(read, (void *)thread_num_ptr);
  }
...
/*
  ...
  for (int blockno = 0; blockno < num_blocks; blockno++) {
    struct threads_space* ts = malloc (sizeof(struct threads_space));
    ts->buf = malloc[8];
    ts->numBlocks = i;
    threads[i] = uthread_create(&read, ts);
}
  for (int blockno = 0; blockno < num_blocks; blockno++) {
    void *res;
    uthread_join(threads[blockno], &res);
}
