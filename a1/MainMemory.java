...
public class MainMemory extends AbstractMainMemory {
  ...
  @Override public int bytesToInteger (byte byteAtAddrPlus0, byte byteAtAddrPlus1, byte byteAtAddrPlus2, byte byteAtAddrPlus3) {
    return
     ((byteAtAddrPlus0 & 0xFF) << 8 * 3) | ((byteAtAddrPlus1 & 0xFF) << 8 * 2) | ((byteAtAddrPlus2 & 0xFF) << 8 * 1) | ((byteAtAddrPlus3 & 0xFF) << 8 * 0);
  }
  ...
  @Override public byte[] integerToBytes (int i) {
    return new byte[]{ (byte) (i >>> 8 * 3), (byte) (i >>> 8 * 2), (byte) (i >>> 8 * 1), (byte) (i >>> 8 * 0)
    };
  }
  
  /**
   * Fetch a sequence of bytes from memory.xq
  ...
  @Override protected void set (int address, byte[] value) throws InvalidAddressException {
    ...
    // the reason for -1 is because if the address 2 block in array, ie 0,1,
    // then if 2 gets passsed in, it will be 0, 1, 2, which is not what we want
      throw new InvalidAddressException();
    }
